   ...
   //Island migration model
      ...
      //The expectation of the migration rate is m, and can vary between 0 an 2m
   	case RANDOM :   //test ok
			for (i=0; i<size; ++i) {
         	for (j=0; j<size; ++j) {
            	if (i!=j) _data[i][j]=2.0*m*ran3(&lidum);
            }
         }
      	break;
...
//------------------------------------------------------------------------------
int
TMigrationMatrix::compute_total_migration_prob() {
...
int
TMigrationMatrix::get_migration_indices(indices& indx){

   long lidum=1;
   //Second, get the index of the source  and sink populations
   double randnum=ran3(&lidum);
   double tot=0.0;
   for (int i=0; i<_size ; ++i) {
...
//------------------------------------------------------------------------------
istream&
operator>>(istream& is, TMigrationMatrix& B)
{
   float mat_elem;
   if (!is)
	{
...
/*
   if (S.EmigrFile[0]) {
		os << "Matrix read from file : " << (*S.EmigrFile)
   		<< " (" << S._size << "x" << S._size << ")\n\n";
   }
   else {
   	os << "Migration matrix size : "
      	<< " (" << S._size << "x" << S._size << ")\n\n";
   }
*/
	S.display_matrix(os);
