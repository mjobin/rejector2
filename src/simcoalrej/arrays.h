...
#include <stdio.h> //MJJ 10/1/12
#include <stdlib.h> //MJJ 10/1/12
#ifndef NULL
#define NULL    0
#endif
...
class MY_TArrayAsVector{
	...
	public:
		...
		~MY_TArrayAsVector(){
      //cout << "enter MY_TArrayAsVector destructor";
      ...
      //T* curr() const {return &_array[0];}
      
      T* elem(int i) const {return & _array[i];}   // Function add for recombination
...
template<typename T>
long MY_TSArrayAsVector<T>::Add(const T& x)
{
	if(_used==_size){
		long new_size=_size+_step;
		T* cop;
...
//right place:
   ...
   long i;
	...
	return (i+1); //stef_28_3 //the element was inserted at posision (i+1)
};
